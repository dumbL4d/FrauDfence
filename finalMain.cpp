...
int unusualSpendingPatterns(vector<transaction> &transactions)
{
    ...
    int creditLimit = 200000; // Credit Limit set to 200000 #Change if required
    ...
    for (const auto &txn : transactions)
    {
        ...
        if (dailySpending[date] > 500.00)
            return 2; // Alerts fraud if daily spending limit exceeds 1000.
        totalSpending += txn.amount;
    }

    if (totalSpending >= creditLimit)
        return 1; // Alerts fraud if total spending exceeds credit limit
